...
static uae_u8* allocate_absolute(uae_u32 addr, uae_u32 size) { return calloc(1, size); }
...
static uae_u32 tosuper(uae_u32 v) { return 0; }
static void touser(uae_u32 v) {}
static uae_u32 exceptiontable000, exceptiontable010, exceptiontable020, exceptiontablefpu;
static uae_u32 testexit(void) { return 0; }
static uae_u32 setvbr(uae_u32 v) { return 0; }
static uae_u32 get_cpu_model(void) { return 0; }
static void setcpu(uae_u32 v, uae_u32* s, uae_u32* d) {}
static void flushcache(uae_u32 v) {}
static void* error_vector;
#else

static void xmemcpy(void* d, void* s, int size) { __builtin_memcpy(d, s, size); }
...
static void endinfo(void) {
    ...
    for (int i = 0; i < 4 * 2; i += 2) {
        if (!is_valid_test_addr_read((uae_u32)(&p[i]))) break;
...
static void start_test(void) {
    ...
    if (test_low_memory_start != 0xffffffff)
        safe_memcpy(low_memory_back + low_memory_offset, low_memory + low_memory_offset,
                    low_memory_size - low_memory_offset);

    // always copy exception vectors if 68000
    if (cpu_lvl == 0 && low_memory_offset > 0x08) safe_memcpy(low_memory_back + 8, low_memory + 8, (192 - 2) * 4);
    ...
    if (test_low_memory_start != 0xffffffff)
        safe_memcpy(low_memory + low_memory_offset, low_memory_temp + low_memory_offset,
                    low_memory_size - low_memory_offset);

    if (cpu_lvl == 0 && low_memory_offset > 0x08) safe_memcpy(low_memory + 8, low_memory_temp + 8, (192 - 2) * 4);
...
static void end_test(void) {
    ...
    if (cpu_lvl == 0 && low_memory_offset > 0x08) safe_memcpy(low_memory + 8, low_memory_back + 8, (192 - 2) * 4);
...
static uae_u8* parse_gzip(uae_u8* gzbuf, int* sizep) {
    ...
    uae_u16 v;
    if (gzbuf[0] != 0x1f && gzbuf[1] != 0x8b) return NULL;
    gzbuf += 10;
...
static int set_berr(int mask, int ask) {
    ...
    if (uaemode) {
        if (!mask) {
            ...
            sprintf(tmpbuffer, "dbg \"w 0 %08x %08x B%s%s%s\"", (uae_u32)safe_memory_start,
                    safe_memory_end - safe_memory_start, (mask & 1) ? "R" : "", (mask & 2) ? "W" : "",
                    (mask & 4) ? "P" : "");
        }
    ...
    if (safe_memory_end < p || safe_memory_start >= p + size) {
        *sizep = readdata(p, size, f, unpack, &unpackoffset);
        ...
        if (size > 0 && p < safe_memory_start) {
            ...
            if (readdata(p, size2, f, unpack, &unpackoffset) != size2) goto end;
            p += size2;
            size -= size2;
        }
        ...
        if (size > 0) {
            if (readdata(p, size, f, unpack, &unpackoffset) != size) goto end;
        }
        size = *sizep;
    }
#endif
...
static uae_u32 gl(uae_u8* p) { return (p[0] << 24) | (p[1] << 16) | (p[2] << 8) | (p[3] << 0); }

static uae_u16 gw(uae_u8* p) { return (p[0] << 8) | (p[1] << 0); }
        ...
        case CT_ABSOLUTE_LONG: {
            ...
            if (!nocheck) {
                if (test_low_memory_start != 0xffffffff && (val & addressing_mask) < low_memory_size) {
                    ;  // low memory
                } else if (test_high_memory_start != 0xffffffff && (val & ~addressing_mask) == ~addressing_mask &&
                           val >= 0xfff80000) {
                    ;  // high memory
...
static void out_disasm(uae_u8* mem) {
    ...
    while (lines++ < 7) {
        ...
        if (!(((uae_u32)code) & 1)) {
            ...
            sprintf(outbp, " %s\n", tmpbuffer);
            outbp += strlen(outbp);
            if (v <= 0 || code[offset] == 0x4afc) break;
            while (v > 0) {
                offset++;
                p += 2;
                v--;
...
static void addinfo(void) {
    ...
    if (code[0] == 0x4e73 || code[0] == 0x4e74 || code[0] == 0x4e75 || code[0] == 0x4e77) {
        ...
        addinfo_bytes(" ", (uae_u8*)stackaddr_ptr - SIZE_STORED_ADDRESS_OFFSET, stackaddr_ptr,
...
static void out_regs(struct registers* r, int before) {
    if (before) {
        for (int i = 0; i < 16; i++) {
            ...
            outbp += strlen(outbp);
            sprintf(outbp, "%c%d:%c%08x", i < 8 ? 'D' : 'A', i & 7,
                    test_regs.regs[i] != last_registers.regs[i] ? '*' : ' ', r->regs[i]);
            outbp += strlen(outbp);
        }
        *outbp++ = '\n';
        sprintf(outbp, "SR:%c%04x      PC: %08x ISP: %08x", test_sr != last_registers.sr ? '*' : ' ', test_sr, r->pc,
                r->ssp);
    } else {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (diff) {
                for (int j = 0; j < 4; j++) {
                    int idx = i * 4 + j;
                    if (j > 0) *outbp++ = ' ';
                    sprintf(outbp, "%c%d:%c%08x", idx < 8 ? 'D' : 'A', idx & 7,
        ...
        sprintf(outbp, "SR:%c%04x/%04x PC: %08x ISP: %08x", test_sr != last_registers.sr ? '*' : ' ', test_regs.sr,
                test_regs.expsr, r->pc, r->ssp);
    }
    outbp += strlen(outbp);
...
static uae_u8* validate_exception(struct registers* regs, uae_u8* p, short excnum, short* gotexcnum, short* experr,
            ...
            switch (frame >> 12) {
                ...
                case 8:  // 68010 bus/address error
                {
                    ...
                    if (basicexcept) {
                        ...
                        // this depends on hardware, cpu does dummy read cycle and some hardware returns memory data,
                        // some ignore it.
                        memcpy(alternate_exception1, exc, exclen);
                        memcpy(alternate_exception2, exc, exclen);
                        ...
                        if (excnum == 2 || !is_valid_test_addr_readwrite(fault_addr - 4) ||
                            !is_valid_test_addr_readwrite(fault_addr + 4)) {
    ...
    if (exclen == 0 || *gotexcnum != excnum) return p;
    int err = 0;
...
static int check_cycles(int exc, short extratrace, short extrag2w1) {
    ...
    if (0 || abs(gotcycles - expectedcycles) > cycles_range) {
        ...
        sprintf(outbp, "Got %d cycles (%d + %d) but expected %d (%d + %d) cycles\n", gotcycles,
                gotcycles - exceptioncycles, exceptioncycles, expectedcycles, expectedcycles - exceptioncycles,
                exceptioncycles);
        outbp += strlen(outbp);
        return 0;
    }
...
static uae_u8* validate_test(uae_u8* p, short ignore_errors, short ignore_sr) {
    ...
    for (;;) {
        ...
        if (mode < CT_AREG + 8 && (v & CT_SIZE_MASK) != CT_SIZE_FPU) {
            ...
            if ((val & (sr_undefined_mask & test_ccrignoremask)) !=
                ...
                if (dooutput) {
                    sprintf(outbp, "SR: expected %04x -> %04x but got %04x", test_sr & 0xffff, val & 0xffff,
                            test_regs.sr & 0xffff);
                    outbp += strlen(outbp);
                ...
                if (skipccrchange ||
                    ((val & (sr_undefined_mask & mask)) == (test_regs.sr & (sr_undefined_mask & mask)))) {
                    errflag &= ~(1 << 7);
                }
            }
            sr_changed = 0;
                ...
                if (regs_changed[i]) {
                    if (dooutput) {
                        sprintf(outbp, "%c%d: modified %08x -> %08x but expected no modifications\n", i < 8 ? 'D' : 'A',
                                i & 7, last_registers.regs[i], test_regs.regs[i]);
                        outbp += strlen(outbp);
                    }
                    errflag |= 1 << 0;
                }
            ...
            if (regs_fpuchanged[i]) {
                if (dooutput) {
                    sprintf(outbp, "FP%d: modified %04x-%08x%08x -> %04x-%08x%08x but expected no modifications\n", i,
                            last_registers.fpuregs[i].exp, last_registers.fpuregs[i].m[0],
    ...
    if (excskipccr && errflag == (1 << 7) &&
        (exceptionnum == 2 || exceptionnum == 3 || exceptionnum == 5 || exceptionnum == 6)) {
        errflag = 0;
    }
...
#ifdef AMIGA
static const int interrupt_levels[] = {0, 1, 1, 1, 2, 3, 3, 3, 4, 4, 4, 4, 5, 5, 6, 6, -1};
...
#ifdef M68K
                if (stackcopysize > 0) xmemcpy((void*)regs.ssp, (void*)regs.regs[15], stackcopysize);
#endif
...
static int test_mnemo(const char* in_path, const char* opcode) {
    ...
    printf("Test: %08x-%08x Safe: %08x-%08x\n", test_memory_addr, test_memory_end, (uae_u32)safe_memory_start,
           (uae_u32)safe_memory_end);
    printf("%s (%s):\n", inst_name, group);
...
#define _stricmp strcasecmp
    ...
    for (;;) {
        if (groupd) {
            ...
            if (!groupdr) break;
            if (!isdir(path, groupdr->d_name)) continue;
            if (groupdr->d_name[0] == '.') continue;
            if (strnicmp(cpustr, groupdr->d_name, strlen(cpustr))) continue;
