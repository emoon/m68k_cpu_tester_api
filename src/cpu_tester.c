...
typedef struct M68KTesterInstructionContext {
...
MK68LoadResult M68KCpuTester_load_opcode(const M68KTesterRunSettings* options, const char* opcode, const char* path) {
	...
	*/
	...
	if (context->high_memory.start != 0xffffffff)
	...
	context->opcode_memory = (uint8_t*)context->test_memory.buffer +
		context->opcode_memory_address - context->test_memory.start;
