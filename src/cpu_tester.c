...
static void print_memory(const M68KTesterMemory* memory, const char* name) {
	...
	printf("%s buffer 0x%p\n", name, memory->buffer);
	printf("%s backup 0x%p\n", name, memory->backup);
}
...
void M68KCpuTester_debug_print(const M68KInstructionTesterContext* context) {
    ...
    print_memory(&context->low_memory, "high");
    print_memory(&context->low_memory, "tester");
}
...
MK68LoadResult M68KCpuTester_load_opcode(const M68KTesterRunSettings* options, const char* opcode, const char* path) {
    ...
    metadata->opcode_memory_address = read_u32(f, filename) + context->test_memory.start;
	...
	*/
	...
	if (!options->check_undefined_sr) {
		metadata->sr_undefined_mask = ~metadata->sr_undefined_mask;
	} else {
		metadata->sr_undefined_mask = 0xffff;
	}

	// Calculate the offsets to use
	if (context->low_memory.start != 0xffffffff)
		context->low_memory.offset = context->low_memory.start;
	if (metadata->high_memory.start != 0xffffffff)
		context->high_memory.offset = context->high_memory.start & 0x7fff;
