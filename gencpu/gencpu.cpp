...
static void genflags_normal(int type, int size, const char *value, const char *src, const char *dst)
{
...
static void genflags(int type, int size, const char *value, const char *src, const char *dst)
{
...
static int gence020cycles_fiea (struct instr *curi, int ssize, int dmode)
{
...
static int gence020cycles_ciea (struct instr *curi, int ssize, int dmode)
{
	int h = 0, t = 0, c = 0, oph = 0;
	bool l = ssize == sz_long;
	switch (dmode)
...
static int gence020cycles_fea (int mode)
{
...
static int gence020cycles_cea (struct instr *curi, int mode)
{
	int h = 0, t = 0, c = 0, oph = 0;
	switch (mode)
	{
...
static int gence020cycles_jea (struct instr *curi, int mode)
{
	int h = 0, t = 0, c = 0, oph = 0;
	switch (mode)
	{
...
static void genamode2x (int mode, const char *reg, int size, const char *name, int getv, int movem, int flags, int fetchmode)
{
	char namea[100];
	bool rmw = false;
...
static void genamode2 (int mode, const char *reg, int size, const char *name, int getv, int movem, int flags)
{
	genamode2x (mode, reg, size, name, getv, movem, flags, -1);
}

static void genamode(struct instr *curi, int mode, const char *reg, int size, const char *name, int getv, int movem, int flags)
{
...
static void genamode3 (struct instr *curi, int mode, const char *reg, int size, const char *name, int getv, int movem, int flags)
{
	int oldfixup = mmufixupstate;
...
static void genamodedual(struct instr *curi, int smode, const char *sreg, int ssize, const char *sname, int sgetv, int sflags,
					   int dmode, const char *dreg, int dsize, const char *dname, int dgetv, int dflags)
{
	int subhead = 0;
	bool eadmode = false;
...
static void genastore_2 (const char *from, int mode, const char *reg, int size, const char *to, int store_dir, int flags)
{
	char tmp[100];
...
static void genastore(const char *from, int mode, const char *reg, int size, const char *to)
{
	genastore_2 (from, mode, reg, size, to, 0, 0);
}
static void genastore_tas (const char *from, int mode, const char *reg, int size, const char *to)
{
	genastore_2 (from, mode, reg, size, to, 0, GF_LRMW);
}
static void genastore_cas (const char *from, int mode, const char *reg, int size, const char *to)
{
	genastore_2 (from, mode, reg, size, to, 0, GF_LRMW | GF_NOFAULTPC);
}
// write to addr + 2, write to addr + 0
static void genastore_rev(const char *from, int mode, const char *reg, int size, const char *to)
{
	genastore_2(from, mode, reg, size, to, 1, 0);
}
// write to addr + 2, prefetch, write to addr + 0
static void genastore_rev_prefetch(const char *from, int mode, const char *reg, int size, const char *to)
{
	genastore_2(from, mode, reg, size, to, 2, 0);
}
static void genastore_fc (const char *from, int mode, const char *reg, int size, const char *to)
{
...
static void force_range_for_rox (const char *var, int size)
{
...
static const char *cmask (int size)
{
	switch (size) {
...
static void shift_ce (int dmode, int size)
{
	if (isreg (dmode)) {
...
static char *decodeEA (int mode, int size)
{
	static char buffer[80];
...
static void generate_cpu_test(const char* path, int mode)
{
	char fname[2048];
	...
	sprintf(fname, "%s/cpuemu_%d%s.cpp", path, postfix, extra);
...
#if CPU_TESTER
    if (argc <= 1) {
        printf("gencpu: expected output path\n");
        return -1;
    }

    char tbl_cpp_name[2048];
    char tbl_h_name[2048];
    const char* path = argv[1];

    sprintf(tbl_h_name, "%s/cputbl_test.h", path);
    sprintf(tbl_cpp_name, "%s/cputbl_test.cpp", path);

	using_test = 1;
	headerfile = fopen(tbl_h_name, "wb");
	stblfile = fopen(tbl_cpp_name, "wb");
	generate_stbl = 1;
	generate_cpu_test(path, 0);
	generate_cpu_test(path, 1);
	generate_cpu_test(path, 2);
	generate_cpu_test(path, 3);
	generate_cpu_test(path, 4);
	generate_cpu_test(path, 5);
