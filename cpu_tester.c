...
typedef struct M68KMenemonicMetadata {
	...
	uint32_t test_memory_address;
	// Size of test memory
	uint32_t test_memory_size;
	// End of test memory
	uint32_t test_memory_end;
	...
	// Status register undefined mask
	uint32_t fpu_model;
	// Testing data low address start
	uint32_t test_low_memory_start;
	// Testing data low end
	uint32_t test_low_memory_end;
	// Testing data high address start
	uint32_t test_high_memory_start;
	// Testing data high end
...
void M68KCpuTester_debug_print(const M68KInstructionTesterContext* context) {
	assert(context);
	const M68KMenemonicMetadata* metadata = &context->metadata;
...
MK68LoadResult M68KCpuTester_load_opcode(const char* opcode, const char* path) {
	...
	sprintf(filename, "%s%s/0000.dat", path, opcode);
	...
	M68KMenemonicMetadata* metadata = &context->metadata;
	...
	metadata->test_memory_address = read_u32(f, filename);
	metadata->test_memory_size = read_u32(f, filename);
	metadata->opcode_memory_address = read_u32(f, filename) + metadata->test_memory_address;
	uint32_t flags = read_u32(f, filename);
	metadata->fpu_model = read_u32(f, filename);
	metadata->test_low_memory_start = read_u32(f, filename);
	metadata->test_low_memory_end = read_u32(f, filename);
	metadata->test_high_memory_start = read_u32(f, filename);
	metadata->test_high_memory_end = read_u32(f, filename);
	...
	metadata->test_memory_end = metadata->test_memory_address + metadata->test_memory_size;
